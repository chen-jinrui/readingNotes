...
// (3) 遍历的应用
// 扩展运算符（...）内部使用for...of循环，所以也可以使用 Set 结构
let set = new Set(['red', 'green', 'blue']);
let arr = [...set];
// ['red', 'green', 'blue']
...
// 2. WeakSet
// 含义：WeakSet 结构与 Set 类似，也是不重复的值的集合。但是，它与 Set 有两个区别。
...
// 结束使用该值后，有时会忘记取消引用，导致内存无法释放，进而可能引发内存泄漏。
// WeakSet 里面的引用，都不计入垃圾回收机制，所以就不存在这个问题。因此，WeakSet适合临时存放一组对象，
// 以及存放跟对象绑定的信息。只有这些对象在外部消失，它在WeakSet里面的引用就会自动消失。
...
// 事实上，不仅仅是数组，任何具有 Iterator 接口、且每个成员都是一个双元素的数组数据结构
// 都可以当作 Map 构造函数。这就是说，Set和Map都可以用来生成新的 Map。
const set = new Set([
  ['foo', 1],
  ['bar', 2]
]);
